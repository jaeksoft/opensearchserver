...
class WebCrawlStatusTransaction extends ServletTransaction {

	private final static String TEMPLATE_INDEX = "web_crawl/status.ftl";

	private final WebCrawlRecord webCrawlRecord;
	private final IndexesService indexesService;
	private final TasksService tasksService;

	WebCrawlStatusTransaction(final CrawlerWebServlet servlet, final UUID webCrawlUuid,
	...
	void crawl() throws IOException, ServletException {
		...
		final WebCrawlTaskRecord record =
				WebCrawlTaskRecord.of(webCrawlRecord, indexUuid).creationTime(System.currentTimeMillis()).build();
	...
	void doGet() throws IOException, ServletException {
		...
		final Map<String, WebCrawlTaskRecord> webCrawlTasks = new TreeMap<>();
		tasksService.getActiveTasks(0, 1000, webCrawlRecord.getUuid()).records.forEach(record -> {
			final WebCrawlTaskRecord webTask = (WebCrawlTaskRecord) record;
			webCrawlTasks.put(indexesService.getIndexName(webTask.indexUuid), webTask);
		});
		request.setAttribute("webCrawlRecord", webCrawlRecord);
		request.setAttribute("webCrawlTasks", webCrawlTasks);
