...
public class WriterLucene extends WriterAbstract {
	...
	private long deleteDocumentsNoLock(int[] ids) throws IOException,
			SearchLibException {
	...
	private long deleteDocumentsNoLock(AbstractRequest request)
			throws SearchLibException {
		try {
			int[] ids = null;
			if (request instanceof AbstractSearchRequest) {
				DocSetHits dsh = indexLucene.searchDocSet(
						(AbstractSearchRequest) request, null);
				if (dsh != null)
					ids = dsh.getIds();
			} else if (request instanceof DocumentsRequest) {
				ResultDocuments result = (ResultDocuments) indexLucene
						.request(request);
				if (result != null)
