...
public abstract class AbstractUploadController extends CommonController {
	...
	public abstract class AbstractUpdateThread extends ThreadAbstract<AbstractUpdateThread> {
		...
		protected AbstractUpdateThread(Client client, StreamSource streamSource, String mediaName) {
			super(client, "Upload", null, null, null);
			this.client = client;
		...
		protected abstract int doUpdate() throws SearchLibException, IOException;
	...
	public AbstractUploadController(ScopeAttribute updateScopeAttribute) throws SearchLibException {
		super();
	...
	private Map<Client, List<AbstractUpdateThread>> getUpdateMap() {
		synchronized (this) {
			synchronized (updateScopeAttribute) {
				@SuppressWarnings("unchecked") Map<Client, List<AbstractUpdateThread>> map =
						(Map<Client, List<AbstractUpdateThread>>) getAttribute(updateScopeAttribute);
				if (map == null) {
	...
	protected abstract AbstractUpdateThread newUpdateThread(Client client, StreamSource streamSource, String mediaName);

	private void doMedia(Media media)
			throws XPathExpressionException, NoSuchAlgorithmException, SAXException, IOException,
			ParserConfigurationException, URISyntaxException, SearchLibException, InstantiationException,
		...
		synchronized (this) {
			...
			if (media.inMemory()) {
				if (media.isBinary()) {
					// Memory + Binary
					streamSource = new StreamSource(new ByteArrayInputStream(media.getByteData()));
				} else {
					// Memory + Texte
			...
			AbstractUpdateThread thread = newUpdateThread(client, streamSource, media.getName());
	...
	public void onUpload(@ContextParam(ContextType.BIND_CONTEXT) BindContext ctx)
			throws InterruptedException, XPathExpressionException, NoSuchAlgorithmException,
			ParserConfigurationException, SAXException, IOException, URISyntaxException, SearchLibException,
