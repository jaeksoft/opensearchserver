...
public abstract class CommonRestAPI {
	...
	public CommonRestAPI() {
		...
		for (Class<?> clazz : RestApplication.CLASSES)
			client.register(clazz);
		target = client.target(IntegrationTest.SERVER_URL);
	}
	...
	public <T extends CommonResult> T checkCommonResult(Response response, Class<T> commonResultClass, int httpCode) {
		...
		assertEquals(response.getEntity().toString(), httpCode, response.getStatus());
	...
	public <T extends CommonResult> T checkCommonResult(CloseableHttpResponse response, Class<T> commonResultClass,
			int httpCode) throws UnsupportedOperationException, IOException {
		assertNotNull(response);
		assertEquals(response.getEntity().toString(), httpCode, response.getStatusLine().getStatusCode());
	...
	private SearchResult search(SearchQueryAbstract query, String path) throws IOException {
	...
	public SearchResult searchPattern(SearchPatternQuery query) throws IOException {
		return search(query, "/services/rest/index/" + IntegrationTest.INDEX_NAME + "/search/pattern");
	}

	public SearchResult searchField(SearchFieldQuery query) throws IOException {
		return search(query, "/services/rest/index/" + IntegrationTest.INDEX_NAME + "/search/field");
	}

	public void updateDocuments(String json) throws IOException {
