...
public class UrlManagerBdb extends UrlManager implements SecondaryKeyCreator {
	...
	public class UrlItemTupleBinding extends TupleBinding<UrlItem> {
		...
		protected DatabaseEntry getKey(UrlItem urlItem)
				throws UnsupportedEncodingException {
			DatabaseEntry key = new DatabaseEntry();
			key.setData(urlItem.getUrl().getBytes("UTF-8"));
			return key;
		}

		protected DatabaseEntry getData(UrlItem urlItem) {
			DatabaseEntry key = new DatabaseEntry();
			objectToEntry(urlItem, key);
			return key;
		}
	...
	private SecondaryDatabase urlHost = null;
	private SecondaryDatabase urlFetchStatus = null;
	private SecondaryDatabase urlParserStatus = null;
	private SecondaryDatabase urlIndexStatus = null;
	private SecondaryDatabase urlWhen = null;
	private SecondaryDatabase urlRetry = null;
	...
	public UrlManagerBdb(CrawlDatabaseBdb crawlDatabase)
		...
		try {
			...
			urlHost = createSecondary(dbEnv, "url_host");
			urlFetchStatus = createSecondary(dbEnv, "url_fetch_status");
			urlParserStatus = createSecondary(dbEnv, "url_parser_status");
			urlIndexStatus = createSecondary(dbEnv, "url_index_status");
			urlWhen = createSecondary(dbEnv, "url_when");
			urlRetry = createSecondary(dbEnv, "url_retry");
